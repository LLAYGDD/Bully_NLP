...
save_best_model_file='E:/DeepLearning/bully_code/diyu/model_multi_attention'
...
for textName in sorted(text_data):
    ...
    if os.path.isdir(textpath):
        ...
        for wordname in sorted(os.listdir(textpath)):
            ...
            texts_w.append(new_content)
            f.close()
            labels.append(label_id)
for tes in texts_w,texts:
    print("------>>")
    # print(tes)
print('Found %s texts.' % len([texts,texts_w]))
# import keras.backend as K
# texs=K.concatenate([texts,texts_w],axis=-1)
...
data = pad_sequences(sequences1, maxlen=max_sequence_length)
...
output=SeqSelfAttention(attention_width=15,attention_activation='relu')(embeddings_sequences)
output=SeqSelfAttention(attention_width=15,attention_activation='relu')(output)

output=Flatten()(output)
output=Dense(256,activation='relu',name='dense1')(output)
output=Dense(128,activation='relu',name='dense2')(output)
output=Dense(64,activation='relu',name='dense3')(output)
print(output)
...
checkpoint_filepath = 'E:/DeepLearning/bully_code/diyu/multi_attention.h5'
